...
<template>
  ...
  <my-container class="container-fluid py-5 my_grades">
    <div
      ...
      <div class="card-body">
        <h4>
          Practical grade <span>{{ g.pcl_g }}</span>
        </h4>
        <h4>
          Theoretical grade <span>{{ g.tcl_g }}</span>
        </h4>
        <h4>
          Final grade <span>{{ g.pcl_g + g.tcl_g }}</span>
        </h4>
      </div>
  ...
  <my-container
    class="container-fluid upToDate"
    v-if="updateForm">
    <my-container class="container mx-auto">
      <form
        class="form rounded shadow p-4 bg-white"
        method="get"
        @submit.prevent>
...
<script setup>
  ...
  const { myRoute, myGrade, myGrades, validateForm } = managerMethod();

  const updateForm = ref(false);

  const { id, sub_name, pcl_g, tcl_g } = storeToRefs(myGrade);

  const { grades } = storeToRefs(myGrades);
  ...
  const edit = (i) => {
    fillGradeData(
      i,
      grades.value[i].sub_name,
      grades.value[i].pcl_g,
      grades.value[i].tcl_g,
    );
